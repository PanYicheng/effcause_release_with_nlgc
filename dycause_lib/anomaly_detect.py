...
def anomaly_detect(
    ...
    """Detect the pivot timestamp when the anomaly becomes apparent.

    Params:
        data: 2d array data of shape [T, N] (N variables).
        ...
        mean_interval: the size of sliding window to calculate standard deviation.
            Anomaly scores of the first (mean_interval - 1) timestamps are 0.
        anomaly_proportion: the threshold of anomaly detection.
            When the score is above anomaly_proportion*N, it is seen as an anomaly. 
            Must relates to weight.
        verbose: the debugging print level: 
                    0 (Nothing), 
                    1 (High level info: method, phase, ...), 
    ...
    pivot_time = pivot_time_list[0]
    # print anomaly detection algorithm result
    if verbose and verbose >= 1:
        print(
            ...
            "{space:^10}{name3:>30}:".format(
                pivot_time,
                dither_proportion[pivot_time],
                space="",
                name1="Best pivot time",
                name2="anomaly score",
                name3="alternatives",
            ),
            pivot_time_list[:10],
    ...
    return pivot_time, dither_proportion[pivot_time]
