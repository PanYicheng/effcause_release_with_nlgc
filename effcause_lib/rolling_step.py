"""
Rolling OLS and WLS with Step
Implements an efficient rolling estimator that avoids repeated matrix
multiplication.
"""
from statsmodels.compat.numpy import lstsq
# from pandas.util._decorators import Appender, Substitution, cache_readonly
# The following import statements doesn't work in my environment 
# (statsmodels=0.10.1). 
...
class RollingWLS(object):
    def __init__(
        ...
        *,
        ...
        self._step = int_like(step, "step", optional=True)
...
#         store.params[idx - 1] = params
        ...
        wresid, ssr = self._loglike(params, wy, wx, weights, nobs)
...
#         store.xpxi[idx - 1] = wxpwxi
        store.nvar[idx - 1] = wxpwxi.shape[-1]
...
#             xpxi=np.full((nobs, k, k), np.nan),
            nvar = np.full(nobs, np.nan),
#             xeex=np.full((nobs, k, k), np.nan)
        )
        w = self._window
        first = self._min_nobs if self._expanding else w
        num_trimmed = first if num_trimmed is None else num_trimmed
        ...
        for i in range(first + 1, self._x.shape[0] + 1):
            ...
            if i % reset == 0:
                ...
                # Modification to satisfy the sliding window scheme under data trimming.
                # Because the first `lag` observations are trimmed in the lagged matrix construction,
                # the first window of size `step` is actually of size `step-lag`. Thus, when we move to
                # the next window of size `2*step`, we consider this offset and substract it.
                # Also, the value of `first` is also specifically designed to be `step-lag` for this modification
                # to work.
                # When update this code, please also refer to function `fit_regression_rolling` in effcause_lib/granger.py.
                # @Date: 2021-09-13 by pyc.
                if ((i-first) % self._step != 0):
                    continue
                if not self._is_nan[i - w - 1] and i > w:
                    remove_x = wx[i - w - self._step : i - w]
                    xpx -= remove_x.T @ remove_x
                    xpy -= remove_x.T @ wy[i - w - self._step : i - w]
                    nobs -= self._step
                if not self._is_nan[i - self._step]:
                    add_x = wx[i - self._step : i]
                    xpx += add_x.T @ add_x
                    xpy += add_x.T @ wy[i - self._step : i]
                    nobs += self._step
    ...
    def fit_zyf(
        ...
        """
        Estimate model parameters. Try to mimic Zhang Yifan's implementation while
        preserving the correctness.
        Parameters
        ----------
        method : {'inv', 'lstsq', 'pinv'}
            Method to use when computing the the model parameters.
...
#             xeex=np.full((nobs, k, k), np.nan)
        ...
        for i in range(first + 1, self._x.shape[0] + 1):
            ...
            if i % reset == 0:
                ...
                # When update this code, please also refer to function `fit_regression_rolling` in effcause_lib/granger.py.
                # @Date: 2021-09-13 by pyc.
                if ((i + num_trimmed) % self._step != 0 and i != self._step - num_trimmed):
                    continue
                if not self._is_nan[i - w - 1] and i > w:
                    remove_x = wx[i - w - self._step : i - w]
                    xpx -= remove_x.T @ remove_x
                    ...
                    nobs -= self._step
                if not self._is_nan[i - self._step]:
                    if i == self._step - num_trimmed:
                        add_slice = slice(first, i)
                    else:
                        add_slice = slice(i - self._step, i)
                    add_x = wx[add_slice]
                    xpx += add_x.T @ add_x
                    xpy += add_x.T @ wy[add_slice]
                    nobs += add_slice.stop - add_slice.start
